...
) : TableColumn<T, Double>(header, width, alignment, fixed, dbColumn, valueProvider, formatter) {

    private val regex =
        "^\$|\\s*[<>]?\\s*[-+]?[0-9]+\\.?[0-9]*\\s*|\\s*[-+]?[0-9]*\\.?[0-9]*\\s*\\.\\.\\s*[-+]?[0-9]+\\.?[0-9]*\\s*".toRegex()
...
) : TableColumn<T, Int>(header, width, alignment, fixed, dbColumn, valueProvider, formatter) {
...
) : TableColumn<T, String>(header, width, alignment, fixed, dbColumn, valueProvider, formatter) {

    override fun validFilterDescription(): Boolean = true

    override fun createFilter(): Filter<String> =
        Filter(dbColumn!!, OP.LIKE, listOf(if (filterAsText.contains("%")) filterAsText else "%$filterAsText%"))
}
